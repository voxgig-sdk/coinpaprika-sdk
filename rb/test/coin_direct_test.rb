...
class CoinDirectTest < Minitest::Test
  def test_direct_list_coin
    setup = coin_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-coin", setup[:live] ? "live" : "unit")
    if _should_skip
...
def coin_direct_setup(mockres)
  ...
  if live
    ...
    client = CoinpaprikaSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
