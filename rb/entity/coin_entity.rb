...
class CoinEntity
  ...
  def make
    opts = @_entopts.dup
    CoinEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CoinpaprikaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Coin, Hash] the current Coin data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CoinpaprikaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Coin fields)
  def match_get
  ...
  #
  # @param reqmatch [CoinListMatch, Hash, nil] match filter (any subset of
  #   Coin fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Coin>, Array] the matching Coin items; raises CoinpaprikaError on failure
