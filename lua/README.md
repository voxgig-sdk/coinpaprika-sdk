...
The Lua SDK for the Coinpaprika API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Coin(nil):list(nil, nil)
...
local result, err = client:Coinpaprika(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── coinpaprika_sdk.lua    -- Main SDK module
...
The main module (`coinpaprika_sdk`) exports the SDK constructor
