...
local CoinEntity = {}
CoinEntity.__index = CoinEntity


function CoinEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CoinEntity)
  self._name = "coin"
  self._client = client
...
function CoinEntity:get_name()
  return self._name
end


function CoinEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CoinEntity.new(self._client, opts)
end
...
function CoinEntity:mark_deleted()
  self._deleted = true
end


function CoinEntity:deleted()
  return true == self._deleted
end


function CoinEntity:data_set(args)
  if args ~= nil then
...
function CoinEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CoinEntity:match_set(args)
  if args ~= nil then
...
function CoinEntity:stream(action, args, callopts)
...
function CoinEntity:_run_op(ctx, post_done)
  local utility = self._utility
