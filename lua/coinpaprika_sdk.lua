...
local CoinpaprikaSDK = {}
CoinpaprikaSDK.__index = CoinpaprikaSDK
...
CoinpaprikaSDK._make_feature = _make_feature


function CoinpaprikaSDK.new(options)
  local self = setmetatable({}, CoinpaprikaSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CoinpaprikaSDK:options_map()
...
function CoinpaprikaSDK:get_utility()
  return Utility.copy(self._utility)
end


function CoinpaprikaSDK:get_root_ctx()
  return self._rootctx
end


function CoinpaprikaSDK:prepare(fetchargs)
...
function CoinpaprikaSDK:direct(fetchargs)
...
-- by passing it.
function CoinpaprikaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CoinpaprikaSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CoinpaprikaSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CoinpaprikaSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CoinpaprikaSDK.new(sdkopts)
  sdk.mode = "test"
