...
class CoinpaprikaSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CoinpaprikaHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CoinpaprikaHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CoinpaprikaFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CoinpaprikaFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CoinpaprikaUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CoinpaprikaHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CoinpaprikaHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CoinpaprikaHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CoinpaprikaError($op . "_allow",
                "CoinpaprikaSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CoinpaprikaHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CoinpaprikaError("graphql_error",
                "CoinpaprikaSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Coin($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_coin === null) {
                $this->_coin = new CoinEntity($this, null);
            }
            return $this->_coin;
        }
        return new CoinEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CoinpaprikaSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
