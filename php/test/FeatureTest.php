...
class FtCtrl extends CoinpaprikaControl
{
...
class FtClient
{
    ...
    private CoinpaprikaUtility $utility;

    public function __construct(CoinpaprikaUtility $utility, array $options)
    {
    ...
    public function get_utility(): CoinpaprikaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CoinpaprikaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CoinpaprikaContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CoinpaprikaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CoinpaprikaConfig::make_feature($name);
    ...
    private static function buildUrl(CoinpaprikaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CoinpaprikaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CoinpaprikaResponse($fetched);
        }
    ...
    private function fail(CoinpaprikaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CoinpaprikaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
