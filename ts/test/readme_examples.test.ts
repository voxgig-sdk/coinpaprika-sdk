...
//     (\`new CoinpaprikaSDK(...)\` / \`CoinpaprikaSDK.test(...)\` become
//     \`CoinpaprikaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"coin":{"test01":{"id":"test01"}},"ticker":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CoinpaprikaSDK(...)\` and any
// \`CoinpaprikaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CoinpaprikaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CoinpaprikaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
