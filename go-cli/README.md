...
boru-driven command-line client **and** interactive REPL for the Coinpaprika
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coinpaprika-cli)
make build
...
./coinpaprika-cli list coin
./coinpaprika-cli list ticker

# 5. Override the API base URL for a single call
COINPAPRIKA_BASE=https://api.example.com ./coinpaprika-cli list coin

# 6. No arguments -> interactive REPL
./coinpaprika-cli
coinpaprika> list coin
coinpaprika> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coinpaprika-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/coinpaprika-cli list coin
   ```
...
Run with no arguments to open a REPL (prompt `coinpaprika>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `coin`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/coinpaprika-cli`. |
...
(`./coinpaprika-cli <expr>`) and interactively (the REPL), and expressions compose the
