// Command Coinpaprika-cli is a CLI + REPL for the Coinpaprika SDK,
...
const prompt = "coinpaprika"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "coin ticker"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COINPAPRIKA_APIKEY (api key), COINPAPRIKA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COINPAPRIKA_APIKEY carries the API key and
	// COINPAPRIKA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COINPAPRIKA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COINPAPRIKA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCoinpaprikaSDK(opts)
