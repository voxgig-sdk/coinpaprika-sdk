...
type Args struct {
	Entity string         `json:"entity" jsonschema:"coin | ticker"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CoinpaprikaSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "coinpaprika_list",
		Description: "List records from Coinpaprika. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "coinpaprika_load",
		Description: "Load a single record from Coinpaprika. " +
...
func runOp(client *sdk.CoinpaprikaSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CoinpaprikaSDK, name string) (sdk.CoinpaprikaEntity, error) {
	switch strings.ToLower(name) {
	case "coin":
		return client.Coin(nil), nil
	case "ticker":
