...
[MCP](https://modelcontextprotocol.io) server exposing the Coinpaprika SDK as
two agent tools — `coinpaprika_list` and `coinpaprika_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coinpaprika-mcp)
make build

# 2. Provide credentials via the environment
export COINPAPRIKA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user coinpaprika \
  -- /absolute/path/to/coinpaprika-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./coinpaprika-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// coinpaprika_list: first page of records
{ "entity": "coin" }
{ "entity": "coin", "query": { } }

// coinpaprika_load: one record by id
{ "entity": "coin", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coinpaprika-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user coinpaprika \
     -- "$PWD"/dist/*/coinpaprika-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `coinpaprika_list` and `coinpaprika_load` tools now appear
   in new sessions. Ask the agent to *"list coin using coinpaprika"*
   and it calls `coinpaprika_list` with `{"entity":"coin"}`.
...
```sh
export COINPAPRIKA_APIKEY=sk_live_xxx            # API key
export COINPAPRIKA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `coinpaprika_list` tool
...
### Call the `coinpaprika_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `coinpaprika_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `coinpaprika_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COINPAPRIKA_APIKEY` | API key sent with every request. |
| `COINPAPRIKA_BASE` | Optional override of the API base URL. |
...
```sh
./coinpaprika-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"coinpaprika_load","arguments":{"entity":"coin","query":{"id":1}}}}'
```
