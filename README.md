...
Provides cryptocurrency market data including prices, market cap, volume, and more.

Learn more about Coinpaprika API at [coinpaprika.com](https://coinpaprika.com).
...
This SDK exposes the API as a small set of **semantic entities** — Coin and Ticker — that you
...
const items = await client.Coin().list()
```
...
```go
client := sdk.Test()
result, err := client.Coin(nil).List(
    nil, nil,
)
```
...
| --- | --- | --- |
| **Coin** | The Coin entity (list). | `/coins` |
| **Ticker** | The Ticker entity (list). | `/tickers` |
