...
import (
	...
	omni "github.com/voxgig-sdk/coinpaprika-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CoinpaprikaError); is {
				out := map[string]any{
					"name":    "CoinpaprikaError",
...
func omniCtx(arg any, client *sdk.CoinpaprikaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CoinpaprikaSDK); is {
			client = live
