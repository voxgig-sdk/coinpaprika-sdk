...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("COINPAPRIKA_TEST_LIVE") == "TRUE" ||
		os.Getenv("COINPAPRIKA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("COINPAPRIKA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CoinpaprikaSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CoinpaprikaError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CoinpaprikaSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CoinpaprikaError{Msg: msg, Code: code}
}
