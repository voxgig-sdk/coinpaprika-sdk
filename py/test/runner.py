# Coinpaprika SDK test runner

from __future__ import annotations
import os
import json

from coinpaprika_sdk.utility.voxgig_struct import voxgig_struct as vs


class CoinpaprikaTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CoinpaprikaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CoinpaprikaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CoinpaprikaTestRunner.getenv("COINPAPRIKA_TEST_LIVE")
        override = CoinpaprikaTestRunner.getenv("COINPAPRIKA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CoinpaprikaTestRunner.getenv(key)
        ...
        explain = CoinpaprikaTestRunner.getenv("COINPAPRIKA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["COINPAPRIKA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CoinpaprikaTestRunner._test_control is not None:
            return CoinpaprikaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CoinpaprikaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CoinpaprikaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CoinpaprikaTestRunner.load_test_control()
...
def load_env_local():
    CoinpaprikaTestRunner.load_env_local()


def env_override(m):
    return CoinpaprikaTestRunner.env_override(m)


def entity_data(v):
    return CoinpaprikaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CoinpaprikaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CoinpaprikaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CoinpaprikaTestRunner.load_test_control()


def live_client_options():
    return CoinpaprikaTestRunner.live_client_options()


def live_delay_ms():
    return CoinpaprikaTestRunner.live_delay_ms()
