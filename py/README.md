...
The Python SDK for the Coinpaprika API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Coin(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Coinpaprika(None).load(
    {"id": "test01"}, None
)
