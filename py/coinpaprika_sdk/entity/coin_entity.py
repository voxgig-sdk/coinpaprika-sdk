...
from coinpaprika_sdk.coinpaprika_types import (
    Coin,
    CoinListMatch,
)
...
class CoinEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CoinEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Coin]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Coin().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
