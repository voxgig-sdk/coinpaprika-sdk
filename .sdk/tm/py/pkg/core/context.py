...
from projectname_sdk.core.control import CoinpaprikaControl
...
class CoinpaprikaContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, CoinpaprikaSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CoinpaprikaResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CoinpaprikaResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return CoinpaprikaError(code, msg, self)
