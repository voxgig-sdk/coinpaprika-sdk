...
module CoinpaprikaUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CoinpaprikaResponse.new({})
    ...
    if fetch_err
      ...
      response = CoinpaprikaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CoinpaprikaResponse.new(fetched)
    else
