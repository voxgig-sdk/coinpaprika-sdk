...
{
    private static function utility(): CoinpaprikaUtility
    {
        return new CoinpaprikaUtility();
    }
    ...
    private static function ctx(array $over = []): CoinpaprikaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CoinpaprikaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CoinpaprikaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CoinpaprikaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CoinpaprikaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CoinpaprikaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CoinpaprikaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CoinpaprikaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CoinpaprikaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CoinpaprikaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CoinpaprikaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CoinpaprikaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CoinpaprikaResponse([]),
            'result' => new CoinpaprikaResult([]),
        ]);
        [, $err] = CoinpaprikaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec([]),
            'response' => null,
            'result' => new CoinpaprikaResult([]),
        ]);
        [, $err] = CoinpaprikaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec(['step' => 's']),
            'response' => new CoinpaprikaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CoinpaprikaResult([]),
        ]);
        [, $err] = CoinpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec(['step' => 's']),
            'response' => new CoinpaprikaResponse(self::resp(200, ['v' => 1])),
            'result' => new CoinpaprikaResult([]),
        ]);
        [, $err] = CoinpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec(['step' => 's']),
            'response' => new CoinpaprikaResponse(self::resp(200, ['v' => 2])),
            'result' => new CoinpaprikaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CoinpaprikaMakeResponse::call($ctx);
    ...
    {
        $preset = new CoinpaprikaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec([]),
            'response' => new CoinpaprikaResponse([]),
            'result' => new CoinpaprikaResult([]),
        ...
        [$got, $err] = CoinpaprikaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CoinpaprikaResult([])]);
        [, $err] = CoinpaprikaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CoinpaprikaSpec([]), 'result' => null]);
        [, $err] = CoinpaprikaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CoinpaprikaSpec(['step' => 's']),
            'result' => new CoinpaprikaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CoinpaprikaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CoinpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CoinpaprikaSpec(['step' => 's']),
            'result' => new CoinpaprikaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CoinpaprikaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CoinpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CoinpaprikaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CoinpaprikaSpec([]),
            'result' => new CoinpaprikaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CoinpaprikaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CoinpaprikaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CoinpaprikaError('boom', 'boom');
        $utility->fetcher = function (CoinpaprikaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CoinpaprikaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CoinpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CoinpaprikaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CoinpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CoinpaprikaResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CoinpaprikaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CoinpaprikaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CoinpaprikaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CoinpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CoinpaprikaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CoinpaprikaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CoinpaprikaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CoinpaprikaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CoinpaprikaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CoinpaprikaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CoinpaprikaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CoinpaprikaResult(['ok' => false])]);
        $this->expectException(CoinpaprikaError::class);
        CoinpaprikaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CoinpaprikaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CoinpaprikaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CoinpaprikaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CoinpaprikaBaseFeature();
        $b = new CoinpaprikaBaseFeature();
        CoinpaprikaFeatureAdd::call($ctx, $a);
        CoinpaprikaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CoinpaprikaBaseFeature
    {
        $f = new CoinpaprikaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CoinpaprikaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CoinpaprikaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CoinpaprikaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CoinpaprikaPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CoinpaprikaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CoinpaprikaResponse(['headers' => null]),
            'result' => new CoinpaprikaResult([]),
        ]);
        CoinpaprikaResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CoinpaprikaResultBody::call($ctx);
